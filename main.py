...
# --- ADMIN CONFIGURATION ---
# ржЖржкржирж╛рж░ ржЯрзЗрж▓рж┐ржЧрзНрж░рж╛ржо ржЗржЙржЬрж╛рж░ ржЖржЗржбрж┐ ржПржЦрж╛ржирзЗ ржпрзЛржЧ ржХрж░рзБржиред @userinfobot ржерзЗржХрзЗ ржЖржЗржбрж┐ ржирж┐ржиред
ADMIN_IDS = [int(admin_id) for admin_id in os.environ.get("ADMIN_IDS", "").split(',') if admin_id]

# --- USER VERIFICATION CONFIGURATION ---
VERIFY_USER = True 
JOIN_LINKS = [
    {'name': 'ЁЯУв Our Channel', 'url': 'https://t.me/your_channel_username', 'id': '@your_channel_username'},
    {'name': 'ЁЯТм Discussion Group', 'url': 'https://t.me/your_group_username', 'id': '@your_group_username'}
]
# --------------------------------
...
async def send_and_schedule_deletion(bot, chat_id, text, delay_seconds):
    try:
        ...
        pass # Ignore if message is already deleted or other errors
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Maintenance check for regular users
    if IS_MAINTENANCE_MODE:
        await update.message.reply_text("Bot is Under Maintenance, please Wait For A while ЁЯФз")
        return
...
async def verify_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if IS_MAINTENANCE_MODE:
        await query.answer("Bot is Under Maintenance ЁЯФз", show_alert=True)
        return
    ...
    try:
        ...
        if is_member:
            ...
            await query.answer("тЭМ You haven't joined all channels/groups yet!", show_alert=True)
    except Exception as e:
...
async def user_button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def assign_new_number(country_key):
        ...
        if not country_data or not country_data.get('numbers'):
            await query.edit_message_text("Sorry, no numbers are available for this option. ЁЯШФ")
            return
...
async def receive_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not numbers:
        await update.message.reply_text("File is empty."); return ConversationHandler.END
...
async def delete_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        for key, data in NUMBER_DATA.items(): message += f"тАв File: <code>{key}.txt</code> (Button: '{data['button_text']}')\n"
        ...
        if key_to_delete in NUMBER_DATA:
            ...
            await update.message.reply_text("тЭМ File not found.")
...
async def pause_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("тЬЕ Bot is now in maintenance mode.")

async def resume_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    global IS_MAINTENANCE_MODE
    if update.effective_user.id not in ADMIN_IDS: await update.message.reply_text("тЭМ Unauthorized."); return
    IS_MAINTENANCE_MODE = False
    await update.message.reply_text("тЬЕ Bot has been resumed.")
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for data in NUMBER_DATA.values():
        message += f"тАв {data['button_text']}\n"
    await update.message.reply_text(message, parse_mode="HTML")
        ...
        try:
            ...
            for sms in reversed(messages):
                ...
                if incoming_number in number_to_user_map:
                    ...
                    await app.bot.send_message(chat_id=target_chat_id, text="Your number has been used and is now released. Select a new one for your next task. ЁЯМН")
        except Exception as e:
